...
export interface Issue {
  ...
  urgent?: boolean;
  /**
   * An emergency in the strict, statutory sense — water, electricity, or
   * heating-season heat off, or something life-threatening. Only these are given
   * the emergency service number. A line that rings for a stuck lock is a line
   * that goes unanswered for a gas leak.
   */
  statutory?: boolean;
  /** Statutory only during the heating season (October to April). */
  winterOnly?: boolean;
...
/**
 * Operating text for a statutory emergency, used verbatim.
 *
 * One string, never paraphrased: a number that comes back slightly different
 * each time is a number nobody trusts. Change it here and every path that hands
 * it out — the form, the thread, and both bot engines — follows.
 */
export const EMERGENCY_CONTACT = "Call Dan at (510) 396-1242 for emergency service.";

/** For urgent issues that are not statutory: escalated, but not a call-out. */
const ON_THE_LIST =
  "This is flagged urgent and goes to your landlord straight away, without troubleshooting.";
...
export const INTAKE: IssueGroup[] = [
  { id: "plumbing", name: "Plumbing", eg: "Leaks, clogs, toilets, hot water", issues: [
    ...
    { id: "nowater", name: "No water at all", eg: "Nothing from any tap in the unit",
      category: "plumbing", priority: "urgent", urgent: true, statutory: true, whenMatters: false,
      emergency: { title: "Check whether it is just you", steps: [
        "Try a second tap in another room, hot and cold, to confirm it is the whole unit.",
        "Ask a neighbour or check the lobby for a notice — a building shut-off is handled "
        + "differently from a fault in your unit.",
        EMERGENCY_CONTACT,
      ] },
      questions: ["Is it every tap, or only some?", "Has a neighbour got water?"],
      tips: [] },
    { id: "pressure", name: "Low water pressure", eg: "Weak flow at one tap or several",
      category: "plumbing", priority: "normal", whenMatters: true,
      questions: [
  ...
  { id: "electrical", name: "Electrical", eg: "Outlets, breakers, lights", issues: [
    ...
    { id: "sparks", name: "Sparks, burning smell, or hot outlet", eg: "Any sign of overheating",
      category: "electrical", priority: "urgent", urgent: true, statutory: true, whenMatters: false,
      emergency: { title: "Stop using that outlet or fixture", steps: [
        ...
        `If you see smoke or flames, get out and call emergency services. Otherwise: ${EMERGENCY_CONTACT}`,
      ] },
    ...
    { id: "nopower", name: "No power in the whole unit", eg: "Everything off at once",
      category: "electrical", priority: "urgent", urgent: true, statutory: true, whenMatters: true,
      emergency: { title: "Check the main breaker, then call", steps: [
        "Look at the main breaker at the top of the panel. If it is midway between ON and OFF, "
        + "push it fully OFF and back ON.",
        "If the hallway or your neighbours are also dark, it is a utility outage — check the "
        + "utility's outage map before anything else.",
        EMERGENCY_CONTACT,
      ] },
      questions: [
  ...
  { id: "hvac", name: "Heating & cooling", eg: "No heat, no AC, thermostat", issues: [
    { id: "noheat", name: "No heat", eg: "Radiator or vents cold",
      category: "hvac", priority: "urgent", urgent: true, statutory: true, winterOnly: true,
      whenMatters: true,
      // No closing line here: whether heat being off is a statutory emergency
      // depends on the date, so `emergencyFor` supplies the last step.
      emergency: { title: "Try the thermostat, then call", steps: [
        "Set the thermostat to HEAT and at least 5 degrees above the room, then wait 10 minutes.",
        "If the display is blank, replace the batteries — most take two AA.",
      ] },
      questions: [
    ...
    { id: "gas", name: "Gas smell", eg: "Rotten-egg odour anywhere",
      ...
      emergency: { title: "Leave the unit now", steps: [
        ...
        `From outside, call the gas utility's emergency line first. Then: ${EMERGENCY_CONTACT}`,
      ] },
  ...
  { id: "doors", name: "Doors, windows & locks", eg: "Locks, latches, glass, screens", issues: [
    ...
    { id: "lockout", name: "Locked out or key won't turn", eg: "Can't get in",
      ...
      tips: [
        ...
        "For a lockout, call the office during hours. Out of hours, raise it here and it goes to your landlord as urgent.",
      ] },
...
/**
 * Whether this issue is a statutory emergency *today*.
 *
 * Only heat is seasonal — it is one of the four named conditions between October
 * and April, and an ordinary urgent repair the rest of the year. Everything else
 * that carries the flag carries it year round.
 */
export function isStatutoryIssue(issue: Issue | null, at: Date = new Date()): boolean {
  if (!issue?.statutory) return false;
  return !issue.winterOnly || inHeatingSeason(at);
}

/**
 * The "what to do right now" block, with its closing line settled for today.
 *
 * A block whose steps already name the number keeps them as written; one that
 * does not gets either the call-out line or the escalation line, depending on
 * whether this counts as statutory on the date asked about.
 */
export function emergencyFor(issue: Issue | null, at: Date = new Date()) {
  if (!issue?.emergency) return null;
  const { title, steps } = issue.emergency;
  if (steps.some((step) => step.includes(EMERGENCY_CONTACT))) return { title, steps };
  return {
    title,
    steps: [...steps, isStatutoryIssue(issue, at) ? EMERGENCY_CONTACT : ON_THE_LIST],
  };
}
